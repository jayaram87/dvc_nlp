...
def main(config_path, params_path):
    ...
    model = RandomForestClassifier(n_estimators=n_est, min_samples_split=min_split, n_jobs=-1, random_state=seed)
    model.fit(X, labels)
...
if __name__ == '__main__':
    ...
    args.add_argument("--configs", "-c", default="configs/configs.yaml")
    args.add_argument("--params", "-p", default="params.yaml")
    parsed_args = args.parse_args()
    try:
        logging.info("\n********************")
        logging.info(f">>>>> stage {STAGE} started <<<<<")
        main(parsed_args.configs, parsed_args.params)
